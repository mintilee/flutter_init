...
class AuthInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) async {
    final String? accessToken = await UserLoginStore().getToken();
    ...
    if (appinfo!.version.isNotEmpty) {
      options.headers["version"] = appinfo.version;
    }
    // 设置来源
    options.headers["versionOrigin"] = kIsWeb
        ? 3
        : Platform.isIOS
            ? 2
            : 1;
...
class TokenInterceptor extends QueuedInterceptor {
  ...
  Future<String?> getToken() async {
    ...
    // 此段先注释掉 params['refresh_token'] = SpUtil.getString(Constant.refreshToken).nullSafe;
    try {
      _tokenDio ??= Dio();
  ...
  Future<void> onResponse(Response<dynamic> response, ResponseInterceptorHandler handler) async {
    ...
    if (response.statusCode == ExceptionHandle.unauthorized) {
      ...
      // 此段先注释掉 SpUtil.putString(Constant.accessToken, accessToken.nullSafe);
...
class LoggingInterceptor extends Interceptor {
  ...
  void onResponse(Response<dynamic> response, ResponseInterceptorHandler handler) {
    ...
    if (response.statusCode == ExceptionHandle.success) {
      Log.d('ResponseCode1: ${response.statusCode}');
    } else {
      Log.d('ResponseCode2: ${response.statusCode}');
    }
    // 输出结果
